...
class DBZHUFFMAN(Compression):
    """
        Class to manipulate DBZHUFFMAN Compression
    ...
    def __init__(self, input_data):
        super(DBZHUFFMAN, self).__init__(input_data)

    def decompress(self, offset=0, lookup_table=0):
        self.DATA.ENDIAN = '<'
        self._output = bytearray()
        self.DATA.set_offset(offset)
        _readed = self.DATA.read_8()
        bits = 7
        lookup_table_current = lookup_table
        print(hex(_readed))
        while offset != 0x23FA1:
            try:
                if bits == -1:
                    bits = 7
                    offset+=1
                    ...
                    _readed = self.DATA.read_8()
                print('B',bits)
                self.DATA.set_offset(lookup_table_current)
                leaf = (_readed >> bits)&0x1
                left = self.DATA.read_8()
                if left == 0x00:
                    decoded = self.DATA.read_8()
                    ...
                    if leaf == 0:
                        print('L', hex(left))
                        lookup_table_current += left
                    else:
                        right = self.DATA.read_8()
                        print('R', hex(right)) 
                        lookup_table_current += right
                    bits-=1
            except:
                break
        return self._output
